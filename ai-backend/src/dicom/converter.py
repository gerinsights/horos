...
async def dicom_to_volume(
    ...
    # Extract DICOM files from ZIP into a temp directory-like structure
    dicom_files = []
    source_datasets = []
    ...
    with tempfile.TemporaryDirectory(prefix="dicom_") as tmpdir:
        ...
        # Also load source datasets for DICOM output metadata
        import pydicom
...
def volume_to_sitk(volume: VolumeData) -> sitk.Image:
    """Convert VolumeData back to a SimpleITK Image (for preprocessing ops)."""
    image = sitk.GetImageFromArray(volume.array)
    # Spacing back to SimpleITK (x, y, z) order

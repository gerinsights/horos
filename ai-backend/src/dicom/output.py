...
- RTSTRUCT: Interactive contour overlays (Horos has built-in rendering)
...
def create_rtstruct(
    ...
    Uses rt-utils-style contour extraction: finds contours on each slice
    of the label map and encodes them as CLOSED_PLANAR contours in
    DICOM patient coordinate space.
    ...
    ds.Manufacturer = "Neuro DICOM AI"
    ...
    for label_id, label_name in result.labels.items():
        ...
        for slice_idx in range(result.mask.shape[0]):
            ...
            for contour_points in contours:
                ...
                # Convert pixel coords (row, col) to DICOM patient coords
                dicom_points = []
...
def create_secondary_captures(
    ...
    for slice_idx in range(min(result.mask.shape[0], len(source_datasets))):
        ...
        if slice_mask.max() == 0:
            continue  # No segmentation on this slice

        src_ds = source_datasets[slice_idx]

        # Get source pixel data as grayscale
        try:
        ...
        # Handle multi-value WindowCenter/Width
        ...
        # Build Secondary Capture dataset
        sc = Dataset()
        ...
        sc.Modality = "OT"
        sc.Manufacturer = "Neuro DICOM AI"
...
def create_dicom_seg(
    ...
    # Create multi-label binary mask array: (num_labels, D, H, W)
    ...
    # Transpose to (D, H, W, num_labels) for highdicom

...
def label_anatomical_regions(
    ...
    # Find the bounding box of vessel voxels
    coords = np.argwhere(vessel_mask > 0)
    z_min, y_min, x_min = coords.min(axis=0)
    z_max, y_max, x_max = coords.max(axis=0)

    z_range = z_max - z_min + 1
    z_split = z_min + int(z_range * 0.35)  # Lower 35% = extracranial
    ...
    # Split extracranial by X (left/right) and Y (anterior/posterior)
    x_coords = np.arange(vessel_mask.shape[2])[None, None, :]

    # Anterior (carotid) vs posterior (vertebral) split
...
def smooth_mask(mask: np.ndarray, iterations: int = 1) -> np.ndarray:
    ...
    Useful before converting to RTSTRUCT contours.

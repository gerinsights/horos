#!/usr/bin/env bash
# Generate TLS certificates for the segmentation backend stack.
...
if [ ! -f "$CERT_DIR/ca.key" ]; then
    ...
    openssl req -new -x509 -sha384 \
        ...
        -out "$CERT_DIR/ca.crt" \
        -days "$DAYS" \
        -subj "/C=US/O=DICOM Segmentation Backend/CN=Segmentation CA"
else
...
openssl req -new -sha384 \
    ...
    -subj "/C=US/O=DICOM Segmentation Backend/CN=$CN"

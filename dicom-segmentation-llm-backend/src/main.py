"""FastAPI application for the DICOM segmentation backend."""
...
async def lifespan(app: FastAPI) -> AsyncIterator[None]:
    ...
    logger.info("DICOM segmentation backend starting")
    logger.info("Device: %s", device_info)
    logger.info("Orthanc URL: %s", settings.orthanc.url)
    yield
    logger.info("DICOM segmentation backend shutting down")


app = FastAPI(
    title="DICOM Segmentation Backend",
    description="AI segmentation backend with LLM integration — GPU-agnostic (ROCm/CUDA/CPU)",
    version="0.1.0",
    lifespan=lifespan,
)

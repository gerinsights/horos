"""MCP tool implementations for the DICOM segmentation backend."""
...
async def dicom_query_study(
    patient_name: str = "",
    patient_id: str = "",
    study_date: str = "",
    modality: str = "",
) -> dict[str, Any]:
    """Query Orthanc for studies matching criteria."""
    query: dict[str, str] = {}
    if patient_name:
...
async def dicom_get_series(study_id: str) -> dict[str, Any]:
...
async def dicom_trigger_segmentation(study_id: str, pipeline: str = "cta") -> dict[str, Any]:
...
async def dicom_pipeline_status(job_id: str) -> dict[str, Any]:
...
async def dicom_list_models() -> dict[str, Any]:
...
async def dicom_get_config() -> dict[str, Any]:
...
async def dicom_architecture_guide(subsystem: str = "overview") -> dict[str, str]:
    """Return architecture documentation for a specific subsystem."""
    uri = f"segmentation://{subsystem}" if not subsystem.startswith("segmentation://") else subsystem
...
async def dicom_logs(service: str = "ai", lines: int = 50) -> dict[str, Any]:
...
async def dicom_llm_models() -> dict[str, Any]:
